...
def getTeamBoxPasses(events_df, teamId, team, pitch_color, cmap):
    """
    Parameters
    ----------
    ...
    Returns
    -------
    Pitch Plot showing both successful and failed box passes along with total pass counts.
    """
    ...
    # Separate successful and unsuccessful box passes
    successful_box_passes = box_passes.loc[box_passes['outcomeType'] == 'Successful'].reset_index(drop=True)
    failed_box_passes = box_passes.loc[box_passes['outcomeType'] != 'Successful'].reset_index(drop=True)
    
    # Get the total number of successful and failed passes
    num_successful = len(successful_box_passes)
    num_failed = len(failed_box_passes)
    ...
    # Plot the successful box passes
    pitch.lines(successful_box_passes.x / 100 * 120, 80 - successful_box_passes.y / 100 * 80,
                ...
                lw=5, cmap=cmap, label='Successful Box Passes', ax=ax)
    
    # Plot the failed box passes
    pitch.lines(failed_box_passes.x / 100 * 120, 80 - failed_box_passes.y / 100 * 80,
                failed_box_passes.endX / 100 * 120, 80 - failed_box_passes.endY / 100 * 80,
                lw=3, color='red', linestyle='--', label='Failed Box Passes', ax=ax)
    
    # Set the title
    fig.suptitle(f'Box Passes - {team}', y=.95, fontsize=15)
    ...
    # Set legend
    ax.legend(facecolor=pitch_color, edgecolor='None', fontsize=8, loc='lower center', handlelength=4)
    
    # Display the total successful and failed passes in the corner
    ax.text(110, 85, f'Successful: {num_successful}', fontsize=10, color='green', ha='right', va='top', weight='bold')
    ax.text(110, 90, f'Failed: {num_failed}', fontsize=10, color='red', ha='right', va='top', weight='bold')
